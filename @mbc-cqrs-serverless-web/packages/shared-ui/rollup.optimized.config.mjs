...
// Optimized Terser configuration for production
const terserConfig = {
	compress: {
		drop_console: true,
		drop_debugger: true,
...
export default defineConfig([
	// Main build configuration with optimizations
	{
		input: 'src/index.ts',
		output: [
			{
		...
		plugins: [
			...
			// Resolve node modules with optimizations
			resolve({
				browser: true,
			...
			// PostCSS processing with optimizations
			postcss({
				extract: true,
		...
		treeshake: {
			...
			preserveEntrySignatures: false, // Allow more aggressive tree shaking
		},
		onwarn(warning, warn) {
	...
	// Server-compatible build with optimizations
	{
		input: 'src/server.ts',
		output: [
			{
